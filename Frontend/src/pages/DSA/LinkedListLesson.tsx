...
export default function LinkedListLesson() {
  ...
  const lesson = linkedListLesson.find(
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 w-full">
        ...
        <div className="leetcode-panel rounded-xl p-8 mb-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-700 mb-3">
            <BookOpen size={13} />
            <span>Lesson {lesson.id} of {linkedListLesson.length} • Linked List Modules</span>
          </div>
        ...
        <div className="mt-8 flex items-center justify-between pt-6 border-t border-slate-200">
          <button
            onClick={() => navigate("/learn-linkedlist")}
          ...
          {lesson.id < 10 ? (
            <button
              onClick={() => navigate(`/learn-linkedlist/${lesson.id + 1}`)}
            ...
            <button
              ...
              className="px-6 py-3 bg-cyan-500 hover:bg-cyan-600 text-slate-950 rounded-xl font-bold text-xs shadow-xs border border-cyan-400 flex items-center gap-2 transition"
            >
