...
import { variablesPracticeQuestions, type QuestionItem } from "../../data/variablesPracticeQuestions";
import { Code2, ArrowRight, Sparkles, Search, Filter } from "lucide-react";

export default function VariablesPracticePage() {
  ...
  const filteredQuestions = useMemo(() => {
    return variablesPracticeQuestions.filter((q) => {
      const matchesSearch =
  ...
  const countEasy = variablesPracticeQuestions.filter((q) => q.difficulty === "Easy").length;
  const countMedium = variablesPracticeQuestions.filter((q) => q.difficulty === "Medium").length;
  const countHard = variablesPracticeQuestions.filter((q) => q.difficulty === "Hard").length;
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 w-full flex-1">
        ...
        <div className="leetcode-panel rounded-xl p-8 mb-8">
          ...
          <h1 className="font-heading text-3xl font-extrabold text-slate-900 sm:text-4xl">
            Variables & Math Practice Missions
          </h1>

          <p className="text-slate-600 text-sm mt-1 font-medium max-w-2xl">
            Solve curated basic data types & mathematical algorithm problems including GCD, LCM, Segmented Sieve, Modular Exponentiation, and Fibonacci calculations.
          </p>
        </div>
        ...
        <div className="leetcode-panel rounded-xl p-4 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 bg-slate-50 pl-10 pr-4 py-2 text-xs font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition"
            />
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
            ...
            >
              All ({variablesPracticeQuestions.length})
            </button>
            <button
        ...
        <div className="space-y-3.5">
          {filteredQuestions.map((question: QuestionItem) => (
            <div
              key={question.id}
              className="leetcode-card rounded-xl p-5 border border-slate-200 bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xs hover:border-blue-500/50 transition-all group"
            >
