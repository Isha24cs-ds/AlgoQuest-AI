...
import { stringsPracticeQuestions, type QuestionItem } from "../../data/stringsPracticeQuestions";
...
export default function StringsPracticePage() {
  ...
  const filteredQuestions = useMemo(() => {
    return stringsPracticeQuestions.filter((q) => {
      const matchesSearch =
  ...
  const countEasy = stringsPracticeQuestions.filter((q) => q.difficulty === "Easy").length;
  const countMedium = stringsPracticeQuestions.filter((q) => q.difficulty === "Medium").length;
  const countHard = stringsPracticeQuestions.filter((q) => q.difficulty === "Hard").length;
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 w-full flex-1">
        ...
        <div className="leetcode-panel rounded-xl p-8 mb-8">
          ...
          <h1 className="font-heading text-3xl font-extrabold text-slate-900 sm:text-4xl">
            Strings Practice Missions
          </h1>

          <p className="text-slate-600 text-sm mt-1 font-medium max-w-2xl">
            Solve 30 curated String interview problems covering anagrams, palindromes, sliding window, KMP algorithm, and dynamic programming edit distance.
          </p>
        </div>
        ...
        <div className="leetcode-panel rounded-xl p-4 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
            ...
            >
              All ({stringsPracticeQuestions.length})
            </button>
            <button
