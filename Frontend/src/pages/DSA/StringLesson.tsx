      ...
      <main className="mx-auto max-w-5xl px-6 py-10 w-full">
        ...
        <div className="leetcode-panel rounded-xl p-8 mb-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 mb-3">
            <BookOpen size={13} />
            <span>Lesson {lesson.id} of {stringsLesson.length} • String Modules</span>
          </div>
        ...
        <div className="mt-8 flex items-center justify-between pt-6 border-t border-slate-200">
          <button
            onClick={() => navigate("/learn-strings")}
          ...
          {lesson.id < 10 ? (
            <button
              onClick={() => navigate(`/learn-strings/${lesson.id + 1}`)}
            ...
            <button
              ...
              className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-slate-950 rounded-xl font-bold text-xs shadow-xs border border-emerald-400 flex items-center gap-2 transition"
            >
